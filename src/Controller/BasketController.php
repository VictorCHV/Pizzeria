...
class BasketController extends AbstractController
{
    ...
    {
        //Recup utilisateur et son panier
        $user= $this->getUser();
        $basket= $user->getBasket();

        //Créer nouvel article à mettre dans le panier
        $article= new Article();
        $article->setQuantity(1);  //  (1) car ajout d'articles UN par UN
        ...
        //sauvegarde du panier dans la DB
        $repository->save($basket, true);

        //redirection vers Controller du panier -> fonction d'affichage
    ...
    public function minus(Article $article, ArticleRepository $repository, BasketRepository $basketRepo): Response
    {
        ...
        if ($article->getQuantity() <= 0) {
            ...
            //mettre à jour la DB via le basketRepo
